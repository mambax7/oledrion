...
<div id="xo-print">
    ...
    <div id="print-info">
        ...
        <div class="pad2"><span class="bold">تاریخ سفارش</span>
            : <{if $order.cmd_create > 0 }><{$order.cmd_create_date}><{else}><{$order.cmd_date|date_format:"%d/%m/%Y"}><{/if}>
        </div>
        <div class="pad2"><span class="bold">نام و نام خانوادگی</span>
            ...
            <{foreach item=product from=$products}>
                <{if isset($product.product_attributes) && count($product.product_attributes) > 0}>
                    <tr>
                        ...
                        <td rowspan="<{math equation='x + y' x=$product.product_attributes_count y=1}>"
                            class="center"><{$product.product_cat_title}> <{$product.product_title}></td>
                        <td rowspan="<{math equation='x + y' x=$product.product_attributes_count y=1}>"
                            class="center"><{$product.product_id}></td>
                        <td class="center" style="border-bottom-color: #fff;"></td>
                        <td class="center" style="border-bottom-color: #fff;"></td>
                        <td class="center" style="border-bottom-color: #fff;"></td>
                        <td rowspan="<{math equation='x + y' x=$product.product_attributes_count y=1}>"
                            class="center"><{$product.product_caddy.caddy_qte}></td>
                        <td rowspan="<{math equation='x + y' x=$product.product_attributes_count y=1}>"
                            class="center"><{$product.product_caddy.caddy_price_fordisplay}></td>
                    </tr>
